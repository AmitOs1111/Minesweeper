...
function onRightClicked(event) {
  ...
  if (currCell.isShow && event.target.innerText === FLAG) {
...
function startTimer() {
  ...
  const millis = Date.now() - gStartTime
  const seconds = Math.floor(millis / 1000)
  const time = formatTimer(seconds)
  document.querySelector('.timer-game').innerText = time
}

function formatTimer(time) {
  if (time < 10) return '0:0' + time
  else if (time >= 10 && time < 60) return '0:' + time
  else if (time >= 60) return time / 60 + ':' + (time % 60)
}
