...
function onUndoGame(elUndo) {
  // console.log('elUndo:', elUndo)
  if (gUndos.length === 0) return
  console.log('gUndos:', gUndos)
  //   const undoBoard = gUndos.pop()
  const undoBoard = gUndos[gUndos.length - 2]
  const compareBoard = gUndos.length === 0 ? '' : gUndos[gUndos.length - 1]

  //   for (var i = 0; i < gBoard.length; i++) {
  //     for (var j = 0; j < gBoard[i].length; j++) {
  //       if (!compareBoard) undoBoard[i][j].isShow = false
  //       else if (undoBoard[i][j].isShow !== compareBoard[i][j].isShow) {
  //         undoBoard[i][j].isShow = compareBoard[i][j].isShow
  //       }
  //     }
  //   }
  renderBoard(undoBoard)
  renderLevelBtn()
}

function setUndoGame() {
  const copyBoard = []
  for (var i = 0; i < gBoard.length; i++) {
    copyBoard[i] = []
    for (var j = 0; j < gBoard[i].length; j++) {
      copyBoard[i][j] = gBoard[i][j]
    }
  }

  gUndos.push(copyBoard)
  renderLevelBtn()
}
...
function renderLevelBtn() {
  var strHTML = `
    <button class="${
      gUndos.length === 0 ? 'empty-undo' : ''
    } border-game undo" onclick="onUndoGame(this)">Undo</button>
    <button class="${
  ...
  const elBtn = document.querySelector('.up-header')
